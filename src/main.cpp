...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "--help" || arg == "-h") {
            ...
            return ok ? 0 : 2;
        } else if (arg == "--hap-crypto-self-test") {
            bool ok = opm::airplay::hap_crypto_self_test();
            return ok ? 0 : 2;
            ...
            return ok ? 0 : 2;
        } else if (arg == "--hap-verify-self-test") {
            bool ok = opm::airplay::hap_verify_self_test();
            return ok ? 0 : 2;
        } else if (arg == "--hap-session-self-test") {
            bool ok = opm::airplay::hap_session_self_test();
            return ok ? 0 : 2;
